...
exports.logLevel = exports.Logger = exports.logToConsole = undefined;
...
var colorconf = {
  ...
  'level': (_level = {}, _defineProperty(_level, logLevel.TRACE, colors.Dim + colors.BgMagenta + colors.FgBlack), _defineProperty(_level, logLevel.DEBUG, colors.Dim + colors.FgBlack + colors.BgGreen), _defineProperty(_level, logLevel.INFO, colors.FgBlue + colors.BgCyan), _defineProperty(_level, logLevel.WARN, colors.Bright + colors.FgYellow + colors.BgBlack + ' ⚠️ '), _defineProperty(_level, logLevel.ERROR, colors.Bright + colors.FgRed + colors.BgBlack + ' ❌'), _defineProperty(_level, logLevel.CRITICAL, colors.Bright + colors.BgRed + colors.FgYellow + colors.Blink + ' ☠️ '), _level),
...
function slackPost(sender, msgShort) {
  ...
  request.post(logger.settings.slackHook, {
    ...
    if (err) logToConsole(logLevel.ERROR, 'slackIntegration', ' 🧨🧨🧨 posting to slack failed! 🧨🧨🧨', err);else if (!err) {
      logToConsole(logLevel.INFO, 'slackIntegration', 'response from webhook: ', { response: response, body: body });
    }
  });
}
