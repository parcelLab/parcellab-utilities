...
function slackPost(sender, msgShort) {
  ...
  request.post(logger.settings.slackHook, {
    ...
    if (!err) {
      logger.info('reponse from webhook: ', { response: response, body: body }, 'slackIntegration');
    }
  });
}
...
logger.critical = function (msg, extra, sender) {
  return loggerPassToConsole('CRITICAL', sender, msg, extra);
};
...
logger.trace = function (msg, extra, sender) {
  return loggerPassToConsole('TRACE', sender, msg, extra);
};
